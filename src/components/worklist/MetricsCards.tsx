...
const MetricCard: React.FC<MetricCardProps> = ({ label, value, timeframe }) => (
  <div className="justify-center items-stretch shadow-[0_2px_3px_0_rgba(57,57,57,0.10)] flex min-w-60 flex-col flex-1 shrink basis-[0%] bg-white px-4 py-2 rounded-lg">
    <div className="text-[#555] text-sm font-normal leading-[22px]">
      {label}
    </div>
    <div className="text-[#181818] text-[32px] font-extrabold leading-10">
      {value}
    </div>
    <div className="text-[#555] text-xs font-medium leading-[18px]">
      {timeframe}
    </div>
  </div>
);
...
export const MetricsCards: React.FC<MetricsCardsProps> = ({ visible = true }) => {
  ...
  return (
    <section className="flex w-full items-stretch gap-2 text-[#555] flex-wrap mt-4 max-md:max-w-full">
