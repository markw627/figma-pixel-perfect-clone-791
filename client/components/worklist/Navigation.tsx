...
export const Navigation: React.FC<NavigationProps> = ({ 
  ...
  return (
    <header className="items-center shadow-[0_2px_4px_0_rgba(25,25,25,0.20)] flex min-h-14 w-full gap-4 flex-wrap bg-brand-navy px-8 py-0 border-b-[rgba(255,255,255,0.00)] border-b border-solid max-md:max-w-full max-md:px-5">
      <div className="items-center self-stretch flex min-w-60 gap-4 font-bold my-auto pr-4">
        ...
        <nav className="self-stretch flex min-w-60 items-center text-sm text-gray-400 leading-loose my-auto">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              ...
              className={`justify-center items-center self-stretch flex min-h-14 gap-1 whitespace-nowrap my-auto px-2 py-4 ${
                activeTab === tab.label
                  ? 'text-white border-b-2 border-b-primary border-solid'
      ...
      <div className="items-center self-stretch flex min-h-14 gap-1 my-auto">
        <div className="items-center self-stretch flex gap-1 my-auto">
          ...
          <button
            onClick={() => setShowUserDropdown(!showUserDropdown)}
            className="text-gray-500 text-xl font-light leading-[22px] self-stretch my-auto hover:text-gray-400"
          >
            keyboard_arrow_down
          </button>
        </div>
        ...
        <button className="self-stretch relative flex flex-col font-normal whitespace-nowrap text-center my-auto hover:opacity-80">
          <div className="text-gray-400 text-2xl leading-none z-0">
            notifications
          </div>
          <div className="absolute z-0 flex text-sm text-white leading-loose w-[22px] h-[22px] bg-status-red px-2 py-0 rounded-[20px] -right-1.5 -top-2">
